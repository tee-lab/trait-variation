# -*- coding: utf-8 -*-
"""AllTrait_Dist_F.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/10aqc-lkazAv2aOKJm6WCKxfPy0xapzka
"""
...
# simulating ODEs for variation in tree death rate (mu)
@njit(fastmath=False)
def iterations_v(b):
  ...
  for g0 in range(len(Gseq)):
          ...
          for t in range(0,timesteps):
              death = u*Si
              G_t= G0 + (np.sum(death) + v*np.sum(Ti) - G0*b*np.sum(Ti)+(0.05 + (0.9-0.05)/(1+np.exp((0.5-G0)/0.005)))*F0-al*G0*F0)*dt
...
def iterations_u(b):
  ...
  for g0 in range(len(Gseq)):
          ...
          for t in range(0,timesteps):
                G_t= G0 + (u*np.sum(Si) + np.sum(v*Ti) - G0*b*np.sum(Ti)+(0.05 + (0.9-0.05)/(1+np.exp((0.5-G0)/0.005)))*F0-al*G0*F0)*dt
                for i in range(0,n_types):
                    s = Si[i]
...
var_list = ["high","low","no"] # Levels of variation
...
df_F.to_csv('F_'+str(n_types)+'_types_'+str(dist)+'_dist_varying_'+str(trait)+'_'+str(var)+'_var.csv')